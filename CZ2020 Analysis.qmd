...
The delineation of commuting zones then uses hierarchical cluster analysis [@Kaufman2009] on this dissimilarity matrix with a cutoff value of .977 utilized in the 2010 delineations as described in @fowler2016 (pp. 274-5). The result is a delineation with `r nCZs` commuting zones. Compared with `r max(county10$OUT10)` in 2010. The delineation includes six commuting zones for Puerto Rico, which was not included in the 2010 delineation. As noted in the technical validation below, alternatives to this cutoff value are worth exploring as it is somewhat arbitrary, but the value does have utility for maintaining consistency across delineations and there is no compelling evidence for the choice of an alternative in the range of descriptive and fit metrics covered below.
...
##Author Contributions Christopher Fowler was the sole author of this publication and the code used to generate it.
